...
struct VioManagerOptions {
  ...
  /// Update options for SuperPoint SuperGlue features (pixel noise and chi2 multiplier)
  UpdaterOptions SuperPointGlue_options;
  ...
  void print_and_load_noise(const std::shared_ptr<ov_core::YamlParser> &parser = nullptr) {
    ...
    if (parser != nullptr) {
      ...
      parser->parse_config("up_superpointglue_sigma_px", SuperPointGlue_options.sigma_pix);
      parser->parse_config("up_superpointglue_chi2_multipler", SuperPointGlue_options.chi2_multipler);
      ...
      SuperPointGlue_options.sigma_pix_sq = std::pow(SuperPointGlue_options.sigma_pix, 2);
    ...
    aruco_options.print();
    PRINT_DEBUG("  Updater SuperPointGlue Feats:\n");
    SuperPointGlue_options.print();
  ...
  /// If should use SuperPoint/SuperGlue tracking
  bool use_SuperPointGlue = true;
  ...
  void print_and_load_trackers(const std::shared_ptr<ov_core::YamlParser> &parser = nullptr) {
    if (parser != nullptr) {
      ...
      parser->parse_config("use_SuperPointGlue", use_SuperPointGlue);
    ...
    PRINT_DEBUG("  - use_SuperPointGlue: %d\n", use_SuperPointGlue);
