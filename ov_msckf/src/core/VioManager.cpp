...
#include "track/TrackSuperPointGlue.h"
...
void VioManager::track_image_and_update(const ov_core::CameraData &message_const) {
  ...
  // If the SuperPointGlue tracker is available, the also pass to it
  // NOTE: SuperPointGlue tracker currently only supports stereo cameras
  if (is_initialized_vio && trackSuperPointGlue != nullptr) {
    trackSuperPointGlue->feed_new_camera(message);
  }
  rT2 = boost::posix_time::microsec_clock::local_time();
...
void VioManager::do_feature_propagate_update(const ov_core::CameraData &message) {
  ...
  // Note:There seems to be a SLAM updater and an MSCKF updater. Do they work together to estimate the state? Is it ok to load SuperPointGlue feats to SLAM?
  if ((int)state->_clones_IMU.size() > state->_options.max_clone_size || (int)state->_clones_IMU.size() > 5) {
    ...
    if (trackSuperPointGlue != nullptr && message.timestamp - startup_time >= params.dt_slam_delay) {
      feats_slam = trackSuperPointGlue->get_feature_database()->features_containing(state->margtimestep(), false, true);
    }
  }
  ...
  for (std::pair<const size_t, std::shared_ptr<Landmark>> &landmark : state->_features_SLAM) {
    ...
    if (trackSuperPointGlue != nullptr) {
      std::shared_ptr<Feature> feat3 = trackSuperPointGlue->get_feature_database()->get_feature(landmark.second->_featid);
      if (feat3 != nullptr)
        feats_slam.push_back(feat3);
    }
  ...
  if (trackSuperPointGlue != nullptr) {
    trackSuperPointGlue->get_feature_database()->cleanup();
  }
  ...
  if ((int)state->_clones_IMU.size() > state->_options.max_clone_size) {
    ...
    if (trackSuperPointGlue != nullptr) {
      trackSuperPointGlue->get_feature_database()->cleanup_measurements(state->margtimestep());
    }
  }
